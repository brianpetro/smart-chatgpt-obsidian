...
const normalize_openwebui_base_url = (value) => {
  ...
  return (
    try_parse(raw_value) ||
    try_parse(`http://${raw_value}`) ||
    DEFAULT_OPENWEBUI_BASE_URL
  );
};

export class SmartOpenWebuiCodeblock extends SmartChatCodeblock {
  /**
    ...
    if (!is_done) {
      ...
      this.mark_done_button_el.onclick = async () => {
        ...
        this.emit_event('chat_codeblock:marked_done', {
          message: 'Marked chat as done.',
          event_source: 'openwebui_codeblock',
          url: link_to_check,
        });
